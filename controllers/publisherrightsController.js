...
const requestpublisherrights = async (req, res) => {
  ...
  const { bio, haspublishedbefore, prevouspublications } = req.body;
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to send the publisher right request" });
  }
};
...
const getpublisherrightsrequest = async (req, res) => {
  try {
    const publisherRightRequests = await Publisheright.find({
      status: "pending",
    ...
    res
      .status(500)
      .json({ error: "Failed to get 'pending' publisher right requests" });
  }
};
  ...
  try {
    ...
    if (foundRequest) {
      ...
      try {
        ...
        res.status(500).json({
          error: "Failed to find ownerof the publisher right acess request",
        });
      }
    ...
    res
      .status(500)
      .json({ error: "Failed to handle publisher right acess request" });
  }
};
module.exports = {
